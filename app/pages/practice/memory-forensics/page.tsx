  ...
  return (
    <div className="flex flex-col w-full min-h-screen">
     ...
     <div className="w-full lg:w-3/5 max-w-screen-xl mx-auto p-4 sm:p-8 text-gray-900 mt-14 animate-fade-in opacity-0 animate-[fadeIn_0.6s_ease-in-out_forwards] overflow-x-hidden">
          ...
          <p className="text-base sm:text-lg text-gray-700 mb-8 mt-8">
          ...
          <p className="text-lg sm:text-lg text-gray-700 mb-8 mt-8">
          ...
          <ul className="list-disc text-lg sm:text-lg text-gray-700 mb-8 ml-8 mt-8 space-y-2">
            <li>
...
# Detect process injection (bash)
volatility -f memory.dmp --profile=Win10x64_19041 malfind
# Network connections analysis (bash)
volatility -f memory.dmp --profile=Win10x64_19041 netscan`}
          ...
          <SyntaxHighlighter language="bash" style={tomorrow}>
            {`# Live memory analysis (bash)
rekall --profile Win10x64_19041 pslist
# Malware detection (bash)
rekall --profile Win10x64_19041 malfind`}
          ...
          <SyntaxHighlighter language="bash" style={tomorrow}>
            {`# Get overview of running processes (bash)
volatility -f memory.dmp pslist | grep -v "System\|smss\|csrss"
# Check for unsigned processes (bash)
volatility -f memory.dmp psxview`}
          ...
          <SyntaxHighlighter language="bash" style={tomorrow}>
            {`# Process tree visualization (bash)
...
volatility -f memory.dmp cmdline`}
          ...
          <SyntaxHighlighter language="bash" style={tomorrow}>
            {`# Detect injected code (bash)
volatility -f memory.dmp malfind -p [PID]
# Check for API hooks (bash)
volatility -f memory.dmp apihooks -p [PID]`}
          ...
          <SyntaxHighlighter language="bash" style={tomorrow}>
            {`# Network connections (bash)
volatility -f memory.dmp netscan | grep [PID]
# DNS queries (bash)
volatility -f memory.dmp dns`}
          </SyntaxHighlighter>
...
def calculate_entropy(data):
    ...
    return entropy`}
          ...
          <SyntaxHighlighter language="bash" style={tomorrow}>
            {`# Scan memory with YARA rules (bash)
volatility -f memory.dmp yarascan -y malware_rules.yar`}
          ...
          <SyntaxHighlighter language="bash" style={tomorrow}>
            {`# Process creation timeline (bash)
